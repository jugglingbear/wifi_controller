"""Unit tests for the Swift ssid_scanner providers (wifi_controller._swift)."""
...
class TestSwiftSsidScannerCurrentSSID:
    ...
    def test_returns_ssid(self) -> None:
        ...
        with patch("wifi_controller._swift.subprocess.run", return_value=mock_result):
    ...
    def test_returns_none_on_timeout(self) -> None:
        with patch(
            "wifi_controller._swift.subprocess.run",
            side_effect=subprocess.TimeoutExpired(cmd="ssid_scanner", timeout=10),
        ):
            assert SwiftSsidScannerCurrentSSID("/fake/bin").get_current_ssid("en0") is None

    def test_returns_none_on_file_not_found(self) -> None:
        with patch("wifi_controller._swift.subprocess.run", side_effect=FileNotFoundError):
            assert SwiftSsidScannerCurrentSSID("/fake/bin").get_current_ssid("en0") is None

    def test_returns_none_on_os_error(self) -> None:
        with patch("wifi_controller._swift.subprocess.run", side_effect=OSError("permission denied")):
            assert SwiftSsidScannerCurrentSSID("/fake/bin").get_current_ssid("en0") is None
...
class TestSwiftSsidScannerConnect:
    ...
    def test_successful_connect(self) -> None:
        ...
        with patch("wifi_controller._swift.subprocess.run", side_effect=[current_result, mock_result]):
    ...
    def test_already_connected_skips(self) -> None:
        ...
        with patch("wifi_controller._swift.subprocess.run", return_value=current_result) as mock_run:
            SwiftSsidScannerConnect("/fake/bin").connect("TestAP 5678", "password", "en0")
    ...
    def test_nonzero_exit_raises(self) -> None:
        ...
        with patch("wifi_controller._swift.subprocess.run", side_effect=[current_result, fail_result]):

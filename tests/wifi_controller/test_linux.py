"""Unit tests for the Linux Wi-Fi providers (wifi_controller._linux)."""
...
class TestNmcliCurrentSSID:
    ...
    def test_available_when_nmcli_exists(self) -> None:
        with patch("wifi_controller._linux.shutil.which", return_value="/usr/bin/nmcli"):
            assert NmcliCurrentSSID().is_available() is True

    def test_unavailable_when_nmcli_missing(self) -> None:
        with patch("wifi_controller._linux.shutil.which", return_value=None):
            assert NmcliCurrentSSID().is_available() is False

    def test_parses_active_ssid(self) -> None:
        output = "yes:TestAP 5678\nno:HomeNet\n"
        with patch("wifi_controller._linux.subprocess.check_output", return_value=output):
    ...
    def test_command_fails(self) -> None:
        with patch(
            "wifi_controller._linux.subprocess.check_output",
            side_effect=subprocess.CalledProcessError(1, "nmcli"),
        ):
            assert NmcliCurrentSSID().get_current_ssid("wlan0") is None

    def test_binary_not_found(self) -> None:
        with patch("wifi_controller._linux.subprocess.check_output", side_effect=FileNotFoundError):
            assert NmcliCurrentSSID().get_current_ssid("wlan0") is None
...
class TestNmcliScan:
    ...
    def test_parses_scan_output(self) -> None:
        ...
        with patch("wifi_controller._linux.subprocess.run"), \
             patch("wifi_controller._linux.subprocess.check_output", return_value=output):
...
class TestNmcliConnect:
    ...
    def test_successful_connect(self) -> None:
        ...
        with patch("wifi_controller._linux.subprocess.run", return_value=mock_result):
    ...
    def test_timeout_raises(self) -> None:
        with patch(
            "wifi_controller._linux.subprocess.run",
    ...
    def test_binary_not_found_raises(self) -> None:
        with patch("wifi_controller._linux.subprocess.run", side_effect=FileNotFoundError):
            with pytest.raises(WiFiConnectionError, match="nmcli failed"):
                NmcliConnect().connect("Net", "pass", "wlan0")
...
class TestNmcliDisconnect:
    ...
    def test_successful_disconnect(self) -> None:
        with patch("wifi_controller._linux.subprocess.run") as mock_run:
...
class TestIwgetidCurrentSSID:
    ...
    def test_available_when_iwgetid_exists(self) -> None:
        with patch("wifi_controller._linux.shutil.which", return_value="/sbin/iwgetid"):
    ...
    def test_parses_ssid(self) -> None:
        with patch("wifi_controller._linux.subprocess.check_output", return_value="TestAP 5678\n"):
            assert IwgetidCurrentSSID().get_current_ssid("wlan0") == "TestAP 5678"

    def test_empty_returns_none(self) -> None:
        with patch("wifi_controller._linux.subprocess.check_output", return_value=""):
